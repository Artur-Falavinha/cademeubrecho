        ...
        {teamMembers.map((member, index) => (
          ...
          >
            <Card className="h-full overflow-hidden border-border/70 bg-white/80 shadow-sm transition-transform duration-500 hover:-translate-y-1 hover:shadow-xl">
              <CardHeader className="flex h-72 items-center justify-center overflow-hidden bg-white p-0">
                {member.image ? (
              ...
              <CardContent className="space-y-3 px-6 py-6">
                <CardTitle className="font-heading text-2xl text-primary-foreground">{member.name}</CardTitle>
                <CardDescription className="text-base text-muted-foreground">{member.bio}</CardDescription>
              </CardContent>
            </Card>
          </motion.article>
        ))}
